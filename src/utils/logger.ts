...
const CURRENT_LOG_LEVEL =
  (env.LOG_LEVEL as LogLevel) || (isDevelopment() ? LogLevel.DEBUG : LogLevel.INFO);
...
interface Logger {
  ...
  error(
    message: string,
    error?: Error | Record<string, any> | null | unknown,
...
const formatDevLog = (
  ...
  if (meta && Object.keys(meta).length > 0) {
    output += `\n${COLORS.dim}Metadata: ${JSON.stringify(meta, null, 2)}${COLORS.reset}`;
  }
  ...
  return {
    debug(message: string, meta?: Record<string, unknown>): void {
      if (shouldLog(LogLevel.DEBUG)) {
        console.debug(formatLogEntry(LogLevel.DEBUG, `[${context}] ${message}`, meta));
      }
    },

    info(message: string, meta?: Record<string, unknown>): void {
      if (shouldLog(LogLevel.INFO)) {
        console.info(formatLogEntry(LogLevel.INFO, `[${context}] ${message}`, meta));
      }
    },

    warn(message: string, meta?: Record<string, unknown>): void {
      if (shouldLog(LogLevel.WARN)) {
        console.warn(formatLogEntry(LogLevel.WARN, `[${context}] ${message}`, meta));
      }
    },

    error(message: string, error?: Error, meta?: Record<string, unknown>): void {
      if (shouldLog(LogLevel.ERROR)) {
        console.error(formatLogEntry(LogLevel.ERROR, `[${context}] ${message}`, meta, error));
      }
    },
  };
};
